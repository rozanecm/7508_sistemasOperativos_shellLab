...
//   to 'arg' 
static char* expand_environ_var(char* arg, int status) {
    if(arg[0] == '$'){
        char* temp_argv;
        if(strcmp(arg+1, "?") == 0){
            char temp[100];
            sprintf(temp, "%i", WEXITSTATUS(status));
            temp_argv = realloc(arg, strlen(temp));
            arg = temp_argv;
            strcpy(arg, temp);
        }else{ 
...
static struct cmd* parse_exec(char* buf_cmd, int status) {
    ...
    while (buf_cmd[idx] != END_STRING) {
        ...
        tok = expand_environ_var(tok, status);
...
// the '&' char
static struct cmd* parse_back(char* buf_cmd, int status) {
...
static struct cmd* parse_cmd(char* buf_cmd, int status) {

    if (strlen(buf_cmd) == 0)
        return NULL;

    if (block_contains(buf_cmd, '|') != -1)
        return parse_line(buf_cmd, status);
...
struct cmd* parse_line(char* buf, int status) {
